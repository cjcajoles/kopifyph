...
`assets/benefit-lowcalorie.jpg` (steaming cup + coffee splash), `assets/benefit-acidreflux.jpg` (stomach shield icon + hand on belly), `assets/benefit-palpitation.jpg` (heart shield icon, "Heart Friendly" badge), `assets/benefit-jitters.jpg` (glowing gold silhouette icon on a coffee-bean background — replaced per client request to match the bloating icon's style), `assets/benefit-bloating.jpg` (glowing gold stomach-outline icon on a coffee-bean background — replaced per client request, the flat black-and-white version looked out of place), and `assets/benefit-nosugar.jpg` (glowing gold crossed-out cupcake icon on a coffee-bean background — replaced per client request to match the bloating/jitters icon style) are real images from client-supplied PDFs. All 6 of 6 benefit-grid icons are now real, and now share a consistent glowing-gold visual style (bloating, jitters, no sugar) except for the 3 photographic cards (low calorie, acid reflux, palpitation).
